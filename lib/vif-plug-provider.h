...
extern "C" {
    ...
    struct vif_plug_port_ctx_in {
         ...
         * decisions based on environmental factors such as settings in the
         * open-vswitch table and datapath type settings on the integration
         * bridge. */
        const struct ovsrec_open_vswitch_table *ovs_table;
        const struct ovsrec_bridge *br_int;

        /* Name of logical port, can be useful for plugging library to track
         * any per port resource initialization. */
        const char *lport_name;

        /* Logical port options, while OVN will forward the contents verbatim
         * from the Southbound database, the convention is for the plugging
         * library to only make decisions based on the vif-plug-* options. */
        struct smap lport_options;

        /* When OVN knows about an existing interface record associated with
         * this lport, these will be filled in with information about it. */
    ...
    struct vif_plug_class {
         ...
         * program startup. This function may be set to null if a VIF plug
         * class needs no initialization at registration time. */
        int (*init)(void);

        /* Called when the VIF plug provider is unregistered, typically at
         * program exit. This function may be set to null if a plug class
         * needs no de-initialization at unregister time. */
        int (*destroy)(void);

        /* Performs periodic work needed by VIF plug provider, if any is
         * necessary. Returns 'true; if anything (i.e. lookup tables) changed, 
         * 'false' otherwise. */
        bool (*run)(struct vif_plug_class *);

        /* Retrieve Interface options this VIF plug provider will maintain.
         * This sset is used to know which items to remove when maintaining
         * the database record. */
         ...
         * prepare for port creation/update. The VIF plug provider
         * implemantation can perform lookup or any per port initialization
         * and should fill vif_plug_port_ctx_out with data required for
         * port/interface creation.  The VIF plug implementation should return
         * 'true' if it wants the caller to create/update a port/interface,
         * 'false' otherwise. Data in the vif_plug_port_ctx_out struct is
         * owned by the VIF plug provider, and a call must be made to the
         * vif_plug_port_ctx_destroy callback to free up any allocations when
         * done with port creation/update. */
