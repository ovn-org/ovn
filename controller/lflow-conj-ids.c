...
struct lflow_to_dps_node {
    ...
    struct uuid lflow_uuid;
    struct ovs_list dps;        /* List of DPs the lflow belongs to. Each node 
                                 * is struct lflow_conj_node.list_node. */
};
...
static struct lflow_conj_node *lflow_conj_ids_find_(struct conj_ids *conj_ids,
                                                    const struct uuid
                                                    *lflow_uuid,
                                                    const struct uuid
                                                    *dp_uuid);
...
lflow_conj_ids_destroy(struct conj_ids *conj_ids)
{
    ...
    HMAP_FOR_EACH_SAFE(conj_id_node, hmap_node, &conj_ids->conj_id_allocations) {
...
void
lflow_conj_ids_clear(struct conj_ids *conj_ids)
{
...
lflow_conj_ids_dump(struct conj_ids *conj_ids, struct ds *out_data)
{
    ...
    HMAP_FOR_EACH(lflow_conj, hmap_node, &conj_ids->lflow_conj_ids) {
        ...
        ds_put_format(out_data,
                      "lflow: " UUID_FMT ", dp: " UUID_FMT ", start: %" PRIu32
                      ", n: %" PRIu32 "%s\n",
                      UUID_ARGS(&lflow_conj->lflow_uuid),
                      UUID_ARGS(&lflow_conj->dp_uuid),
                      lflow_conj->start_conj_id, lflow_conj->n_conjs,
                      has_conflict ? " (*)" : "");
    ...
    if (count != allocated) {
        ds_put_format(out_data,
                      "WARNING: mismatch - %" PRIuSIZE " allocated\n",
                      allocated);
    }
}
...
lflow_conj_ids_find_(struct conj_ids *conj_ids,
                     const struct uuid *lflow_uuid, const struct uuid *dp_uuid)
{
    struct lflow_conj_node *lflow_conj;
