...
ipam_insert_ip(struct ipam_info *info, uint32_t ip)
{
    ...
    if (ip >= info->start_ipv4 && ip < (info->start_ipv4 + info->total_ipv4s)) {
        if (bitmap_is_set(info->allocated_ipv4s, ip - info->start_ipv4)) {
        ...
        bitmap_set1(info->allocated_ipv4s, ip - info->start_ipv4);
    }
    return true;
}
...
init_ipam_ipv6_prefix(const char *ipv6_prefix, struct ipam_info *info)
{
    ...
    /* XXX Since we only accept /64 addresses, why do we even bother with
     * accepting and trying to analyze a user-provided mask? */
    if (strchr(ipv6_prefix, '/')) {
        /* If a prefix length was specified, it must be 64. */
        struct in6_addr mask;
        char *error = ipv6_parse_masked(ipv6_prefix,
        ...
        info->ipv6_prefix_set = ipv6_parse(ipv6_prefix, &info->ipv6_prefix);
               ...
               struct ipam_info *info)
{
    ...
    info->allocated_ipv4s = bitmap_allocate(info->total_ipv4s);
    ...
    /* exclude_ip_list could be in the format - * "10.0.0.4 10.0.0.10
     * 10.0.0.20..10.0.0.50 10.0.0.100..10.0.0.110". */
    lexer_get(&lexer);
    while (lexer.token.type != LEX_T_END) {
        ...
        if (end > start) {
            bitmap_set_multiple(info->allocated_ipv4s,
                                start - info->start_ipv4, end - start, 1);
        } else {
...
ipam_is_duplicate_mac(struct eth_addr *ea, uint64_t mac64, bool warn)
{
    ...
    HMAP_FOR_EACH_WITH_HASH(macam_node, hmap_node, hash_uint64(mac64), &macam) {
